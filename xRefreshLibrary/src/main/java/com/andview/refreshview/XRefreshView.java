...
//            int childHeightSpec = MeasureSpec.makeMeasureSpec(height - lp.topMargin - lp.bottomMargin - paddingTop - paddingBottom,MeasureSpec.EXACTLY);
...
//        if(mHolder.mOffsetY!=0)return;
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (/*!enablePullUp ||*/ mStopingRefresh || !isEnabled() || mIsIntercept) {
                    //M*修复CoordinatorLayout滑动冲突
                    mLastY = (int) ev.getRawY();
                    mLastX = (int) ev.getRawX();
                    //还在刷新时，由下拉刷新控件消耗这个事件 2018.7.24
                    if (mHolder.mOffsetY!=0 && (mPullLoading || mPullRefreshing) && mIsPinnedContentWhenRefreshing) {
                        sendCancelEvent();
                        return true;
                    }
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mHolder.hasHeaderPullDown()) {
                    if (mEnablePullRefresh && !mStopingRefresh && !mPullRefreshing && mHolder.mOffsetY > (mHeaderViewHeight - Utils.dip2px(getContext(), 100))) {
    ...
    private void resetHeaderHeight() {
        ...
        if (mPullRefreshing && (height <= (mHeaderViewHeight - Utils.dip2px(getContext(), 100))) || height == 0) {
            return;
        }
        int offsetY;
        if (mPullRefreshing) {
            offsetY = mHeaderViewHeight - Utils.dip2px(getContext(), 100) - mHolder.mOffsetY;
    ...
    public void moveView(int deltaY) {
        ...
        if(isFixHeaderView){
            if(!mPullRefreshing){
                int fixHeaderAnimHeight = mHeaderViewHeight - Utils.dip2px(getContext(), 100);
                int currTop = mHeaderView.getTop();//当前头部top高度
                int mTop = deltaY + currTop;
                int offset;
                Log.e("test12","bottom:"+mHeaderView.getBottom()+",top:"+currTop+",fixHeaderAnimHeight:"+fixHeaderAnimHeight);
                if(mHolder.mOffsetY>=fixHeaderAnimHeight){
                    offset = -(mHeaderViewHeight - fixHeaderAnimHeight) - currTop;
                }else if(mTop<=-mHeaderViewHeight){
                    offset = -mHeaderViewHeight - currTop;
                }else{
                    offset = deltaY;
                }
...
//        mHeaderView.offsetTopAndBottom(deltaY);
        ...
        if (mRefreshViewListener != null && (mContentView.isTop() || mPullRefreshing)) {
            double headerMovePercent = 1.0 * mHolder.mOffsetY / (mHeaderViewHeight - Utils.dip2px(getContext(), 100));
//            headerMovePercent = headerMovePercent > 1 ? 1 : headerMovePercent;
    ...
    private boolean isFixHeaderView;
    private int fixHeight;

    public void setIsFixHeaderView(boolean isFixHeaderView,int fixHeight){
        this.isFixHeaderView = isFixHeaderView;
...
//                mFooterCallBack.show(true);
    ...
    /**
     * 设置当下拉刷新到刷新完成最小的时间
     * 注:考虑到ui效果不要太小，默认800毫秒
     *
     * @param mMinDuringTime
     */
    public void setMinDuringTime(int mMinDuringTime) {
        if (mMinDuringTime > 800) {
            mPinnedTime = mMinDuringTime;
        }
    }
